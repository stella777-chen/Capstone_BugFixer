...
// import "./DashboardCantierDataTableCardComponent.css";
...
export interface CustomCardProps extends CardBaseProps {
	...
	customStyles?: {
		...
		// Add more targeted styles if needed later
	};
	tooltip?: string;
}
...
const CantierDataTableCardComponent: React.FC<CardProps> = (props) => {
	...
	return (
		<Card
			className={`${isCustom ? "custom-card" : "standard-card"} ${className}`}
			style={{ backgroundColor: hexColor, ...(customStyles.card || {}) }}
			title={tooltip}
		>
			<CardHeader
				image={icon ? <div className="card-icon">{icon}</div> : null}
				header={
					<Text className="card-header-text" style={customStyles.title}>
						{title}
					</Text>
				}
				description={<Caption1 className="card-caption">{subtitle}</Caption1>}
			/>
